...
#define DEBUG_PIN_NODE DT_NODELABEL(debug0)
...
static struct gpio_dt_spec debug_gpio = GPIO_DT_SPEC_GET(DEBUG_PIN_NODE, gpios);
...
//    }
//
//    int ret = gpio_pin_get_dt(&sniff_mosi_gpio);
//    if (ret < 0) {
...
//    if (scrape_buffer_bit_index >= (SPI_SCRAPER_BUFFER_SIZE << 3)) { // equivalent to * 8
...
//        processPunch(scrape_buffer);
        ...
        char buffer_str[SPI_SCRAPER_BUFFER_SIZE * 3 + 1]; // 3 characters per byte (2 for hex and 1 for space) and 1 for null terminator
        for (int i = 0; i < SPI_SCRAPER_BUFFER_SIZE; i++) {
            sprintf(&buffer_str[i * 3], "%02x ", scrape_buffer[i]);
        }
        printk("%s----\n", buffer_str);
    }
    // print out the first byte of the buffer
    printk("%02x\n", scrape_buffer[0]);
...
static void initialize_inputs()
{
    ...
    if (!device_is_ready(debug_gpio.port)) {
        LOG_ERR("Debug pin not ready.");
        return;
    }
    ret = gpio_pin_configure_dt(&debug_gpio, GPIO_OUTPUT);
    if (ret < 0) {
...
void spi_sniffer(void *arg1, void *arg2, void *arg3)
{
    ...
    while (1) {
        ...
        // Update the debug pin only when the clock pin value changes
//        if (clck_state != last_debug_state) {
//            int ret = gpio_pin_set_dt(&debug_gpio, clck_state);
//            if (ret < 0) {
//                LOG_ERR("Error setting debug pin: %d", ret);
//                continue;
//            }
//            last_debug_state = clck_state;
//        }
